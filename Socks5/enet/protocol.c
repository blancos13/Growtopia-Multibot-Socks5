...
enet_protocol_handle_connect(ENetHost* host, ENetProtocolHeader* header, ENetProtocol* command)
{
        ...
        ++currentPeer)
    {
        if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED)
        {
            ...
            else if (host->usingSocks5 == TRUE) {
                if (currentPeer->state != ENET_PEER_STATE_CONNECTING &&
                    currentPeer->proxyaddress.host == host->receivedAddress.host)
                {
                    if (currentPeer->proxyaddress.port == host->receivedAddress.port &&
                        currentPeer->connectID == command->connect.connectID)
                        return NULL;
    ...
    if (host->usingSocks5 == FALSE) {
        ...
        peer->proxyaddress = host->receivedAddress;
    }
...
enet_protocol_handle_incoming_commands(ENetHost* host, ENetEvent* event)
{
    ...
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeader*)0)->sentTime)
        return 0;
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeader*)0)->sentTime && !host->usingNewPacket)
        return 0;
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeaderUbisoft*)0)->sentTime && host->usingNewPacket)
        return 0;
    ...
    if (peerID == ENET_PROTOCOL_MAXIMUM_PEER_ID)
        ...
        else
        {
            ...
            if (host->usingSocks5 == FALSE) {
                ...
                if (peer->state == ENET_PEER_STATE_DISCONNECTED ||
                    peer->state == ENET_PEER_STATE_ZOMBIE ||
                    ((host->receivedAddress.host != peer->proxyaddress.host ||
                        host->receivedAddress.port != peer->proxyaddress.port) &&
                        peer->proxyaddress.host != ENET_HOST_BROADCAST) ||
                    (peer->outgoingPeerID < ENET_PROTOCOL_MAXIMUM_PEER_ID &&
                        sessionID != peer->incomingSessionID))
                    return 0;
            }
    ...
    if (peer != NULL)
    {
        if (host->usingSocks5 == FALSE) {
            ...
            peer->proxyaddress.host = host->receivedAddress.host;
            peer->proxyaddress.port = host->receivedAddress.port;
            peer->incomingDataTotal += host->receivedDataLength;
        }
    }
    ...
    while (currentData < &host->receivedData[host->receivedDataLength])
    {
        ...
        if (command != (unsigned char*)-1 && command->header.command != (unsigned char*)-1) {
            if (command->header.command != NULL && command != NULL) {
                if (peer != NULL &&
...
enet_protocol_receive_incoming_commands(ENetHost* host, ENetEvent* event)
{
    ...
    for (packets = 0; packets < 256; ++packets)
    {
        ...
        /*
        if (host->usingSocks5) {
            enet_uint8 payload[sizeof(ENetSocks5UDPHeader) + sizeof(host->packetData[0])];
            ENetSocks5UDPHeader req;
            req.addr.s_addr = &host->address.host; //destination address
            req.frag = 0;
            req.rsv = 0x00;
            req.type = 0x01;
            req.port = htons(&host->address.port); //destination port  
            memcpy(payload + sizeof(ENetSocks5UDPHeader), host->packetData[0], sizeof(host->packetData[0]));
            buffer.dataLength = sizeof(ENetSocks5UDPHeader) + sizeof(host->packetData[0]);
            buffer.data = payload;
        }
        else {
            */
       ...
       // }
        ...
        if (host->usingSocks5) {
            int requiredLength = (&host->receivedAddress == NULL) ? 0 : 10;
            if (receivedLength < requiredLength)
                return -1;

            if (&host->receivedAddress != NULL && buffer.dataLength < requiredLength)
                return -1;

            host->receivedData = host->packetData[0];
            host->receivedDataLength = receivedLength;

            if (&host->receivedAddress != NULL)
            {
                host->receivedData += 10;
...
enet_protocol_send_outgoing_commands(ENetHost* host, ENetEvent* event, int checkForTimeouts)
{
    ...
    enet_uint8 payload[sizeof(ENetSocks5UDPHeader) + sizeof(ENetProtocolHeaderUbisoft) + sizeof(enet_uint32)];
    ...
    while (host->continueSending)
            ...
            ++currentPeer)
        {
            ...
            if (host->usingSocks5) {
                ENetSocks5UDPHeader req;
                req.addr.s_addr = currentPeer->address.host; //destination address
                req.frag = 0;
                req.rsv = 0x00;
                req.type = 0x01;
                req.port = htons(currentPeer->address.port); //destination port  
                *(ENetSocks5UDPHeader*)payload = req;
                memcpy(payload + 10, host->buffers->data, host->buffers->dataLength);
                host->buffers->dataLength += 10;
                host->buffers->data = payload;
                sentLength = enet_socket_send(host->socket, &currentPeer->address, &currentPeer->proxyaddress, host->buffers, host->bufferCount);
            }
            else {
                sentLength = enet_socket_send(host->socket, &currentPeer->address, NULL, host->buffers, host->bufferCount);
            }
