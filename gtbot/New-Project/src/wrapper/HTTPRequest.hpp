...
namespace http
{
    ...
    inline namespace detail
    {
        ...
        constexpr int getAddressFamily(InternetProtocol internetProtocol)
        {
            return (internetProtocol == InternetProtocol::V4) ? AF_INET :
                ...
                throw ResponseError("Unsupported protocol");
        }
    ...
    public:
                      ...
                      const std::chrono::milliseconds timeout = std::chrono::milliseconds{-1})
        {
            ...
            if (scheme != "http")
                throw ResponseError("Only http scheme supported");
                    ...
                    for (;;)
                    {
                        ...
                        else if (state == State::statusLine) // RFC 7230, 3.1.2. Status Line
                        {
                            ...
                            for (auto beginIterator = line.begin(); beginIterator != line.end();)
                            {
                                ...
                                if (++partNum == 2) response.status = atoi(part.c_str());
                        ...
                        else if (state == State::headers) // RFC 7230, 3.2.  Header Fields
                        {
                            ...
                            {
                                contentLength = strtoul(headerValue.c_str(), NULL, NULL);
                    ...
                    if (chunkedResponse)
                    {
                        for (;;)
                        {
                            ...
                            else
                            {
                                ...
                                expectedChunkSize = strtoul(line.c_str(), nullptr, 16);
                                if (expectedChunkSize == 0)
                                    return response;
                            }
                        }
